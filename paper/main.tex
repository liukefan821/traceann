...
with a $607$\,KiB proof per query, $325\,\mu$s proving ($8.4\times$
...
\noindent\textbf{Keywords:} Verifiable computation; Approximate
nearest neighbor search; Authenticated data structures; Query
integrity; Retrieval-augmented generation; Merkle commitments
...
into int8), \sys{} attains recall@10 $=0.956$ with a $607$\,KiB proof per
...
\begin{itemize}
  ...
  $0.956$ at $607$\,KiB VO, $325\,\mu$s proving, $2.7$\,ms verification;
...
paper depends on: a golden-digest test pins the bytes of a fixed index and query,
which we confirm reproduce identically on x86-64 Linux and aarch64
macOS --- a determinism regression stops the build --- and in debug builds the
...
Two further directions follow naturally. First, extending trace consistency to \emph{filtered} search ---
nearest-neighbor queries under a committed metadata predicate --- is
a natural direction that our deterministic-execution framing should
support. Second, the integer-exact distance kernels are associative by
